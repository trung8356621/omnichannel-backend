...
final class ContentProjectActiveExecutionLifecycleTest extends TestCase
{
    ...
    public function test_false_busy_on_failed_step_does_not_win_as_active(): void
    {
        ...
        $status = $resolver->resolve([
            ...
            'active_execution' => null,
            'workflow_steps' => [
                ['label' => 'Chạy lại bài viết', 'busy' => true, 'status' => 'failed'],
            ],
        ]);
        self::assertSame(ContentProjectArticleRowStatus::CODE_FAILED, $status->code);
    }
    ...
    public function test_finalizer_and_repair_command_exist(): void
    {
        self::assertTrue(class_exists(
            \App\Addons\SeoContentAi\Services\ContentProject\ContentProjectExecutionFinalizer::class
        ));
        self::assertTrue(class_exists(
            \App\Addons\SeoContentAi\Services\ContentProject\ContentProjectActiveExecutionRepairService::class
        ));
        self::assertTrue(class_exists(
        ...
        $cmd = (string) file_get_contents(
            dirname(__DIR__, 2).'/Console/RepairContentProjectActiveExecutionsCommand.php',
        );
        ...
        $repair = (string) file_get_contents(
            dirname(__DIR__, 2).'/Services/ContentProject/ContentProjectActiveExecutionRepairService.php',
        );
