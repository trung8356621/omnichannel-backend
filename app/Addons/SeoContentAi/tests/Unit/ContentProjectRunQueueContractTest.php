...
use App\Addons\SeoContentAi\Support\Queue\ContentProjectQueueRuntimeSafety;
...
final class ContentProjectRunQueueContractTest extends TestCase
{
    ...
    {
        $queueConfigPath = dirname(__DIR__, 5).DIRECTORY_SEPARATOR.'config'.DIRECTORY_SEPARATOR.'queue.php';
    ...
    {
        $envExample = dirname(__DIR__, 5).DIRECTORY_SEPARATOR.'.env.example';
    ...
    public function test_queue_runtime_check_command_is_read_only(): void
    {
        $path = (string) (new ReflectionClass(
            \App\Addons\SeoContentAi\Console\QueueRuntimeCheckCommand::class,
        ))->getFileName();
