...
use App\Addons\SeoContentAi\Services\AgentWorkspace\Execution\Presentation\MemberListPresenter;
use App\Addons\SeoContentAi\Services\AgentWorkspace\Execution\Presentation\SiteInfoPresenter;
...
final class AgentMcpSiteCliFixTest extends TestCase
{
    public function test_mcp_page_is_simple_markdown_documentation(): void
    {
        ...
        // Primary view is HTML docs — raw <pre> only behind optional toggle.
    ...
    public function test_message_refresh_and_append_use_sanitizer(): void
    {
        ...
        $conv = (string) file_get_contents(
            (new ReflectionClass(\App\Addons\SeoContentAi\Services\AgentWorkspace\AgentConversationService::class))->getFileName()
        );
    ...
    public function test_member_commands_use_member_id_not_name(): void
    {
        ...
        $bad = $parser->parse('/project-create --name="X" --month="08/2026" --member-id="Trang Nguyễn"');
    ...
    public function test_site_switch_and_list_handlers_exist(): void
    {
        ...
        self::assertStringContainsString('không thuộc site hiện tại', $source);
    }
    ...
    {
        $js = (string) file_get_contents(dirname(__DIR__, 2).'/resources/js/agent/command-catalog.js');
