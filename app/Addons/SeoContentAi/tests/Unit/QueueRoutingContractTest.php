...
use App\Addons\SeoContentAi\Jobs\DispatchContentProjectAutomationPoliciesJob;
...
final class QueueRoutingContractTest extends TestCase
{
    ...
    public function test_wordpress_external_node_queue_name_is_automation_external(): void
    {
        ...
        $graph = (string) file_get_contents(
            dirname(__DIR__, 2).'/Automation/BusinessHook/Services/AutomationGraphExecutionService.php',
        );
    ...
    public function test_schedule_registers_policy_job_on_automation_policy_queue(): void
    {
        ...
        // Schedule::job($job, $queue) — CallbackEvent has no onQueue().
    ...
    public function test_filament_queued_notifications_resolve_to_connection_default_not_wp_queues(): void
    {
        ...
        $queueConfig = (string) file_get_contents(dirname(__DIR__, 5).'/config/queue.php');
        self::assertStringContainsString("env('DB_QUEUE', 'default')", $queueConfig);

        $notificationSources = [
            dirname(__DIR__, 2).'/Services/SeoNotificationService.php',
            dirname(__DIR__, 2).'/Support/CreateArticleWorkflowNotification.php',
            dirname(__DIR__, 2).'/Services/TeamChatNotificationService.php',
        ];
