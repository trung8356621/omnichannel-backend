...
final class ProductReviewArticleSyncIsolationTest extends TestCase
{
    ...
    public function test_three_action_codes_exist(): void
    {
        ...
        self::assertTrue(class_exists(\App\Addons\SeoContentAi\Automation\BusinessHook\Actions\CreateProductReviewsHookAction::class));
        self::assertTrue(class_exists(\App\Addons\SeoContentAi\Automation\BusinessHook\Actions\SyncProductReviewsToWordPressHookAction::class));
    ...
    public function test_legacy_review_actions_are_safe_noops(): void
    {
        foreach ([
            ...
            $source = (string) file_get_contents(
                dirname(__DIR__, 2).'/Automation/BusinessHook/Actions/'.$file,
            );
