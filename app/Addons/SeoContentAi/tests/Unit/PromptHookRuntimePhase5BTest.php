...
use App\Addons\SeoContentAi\PromptHooks\Exceptions\BudgetExceeded;
...
use App\Addons\SeoContentAi\PromptHooks\Exceptions\InputTooLarge;
...
use App\Addons\SeoContentAi\PromptHooks\Provider\PromptStructuredStrategy;
...
use App\Addons\SeoContentAi\PromptHooks\Spec\PromptHookSpecV01Validator;
...
final class PromptHookRuntimePhase5BTest extends TestCase
{
    ...
    public function test_duplicate_key_version_rejected_on_load(): void
    {
        ...
        try {
            $this->expectException(\App\Addons\SeoContentAi\PromptHooks\Exceptions\PromptHookFailure::class);
            $loader->indexed();
        } finally {
            @unlink($dir.'/a.json');
            @unlink($dir.'/b.json');
    ...
    public function test_previous_outputs_size_reject_without_truncate(): void
    {
        ...
        // Shrink limits via reflection-free: use guard with custom def clone — hydrate small limits
        $loader = $this->loader();
        ...
        $guard = new \App\Addons\SeoContentAi\PromptHooks\Runtime\PromptHookPreviousOutputsGuard;
    ...
    public function test_template_strict_missing_variable(): void
    {
        ...
        // keyword discovery has strict_template_variables false — use hydrate strict
        $loader = $this->loader();
    ...
    public function test_boundary_runtime_avoids_eloquent_filament_wordpress(): void
    {
        ...
        $dirs = [
            dirname(__DIR__, 2).'/PromptHooks/Runtime',
            dirname(__DIR__, 2).'/PromptHooks/Canonical',
            dirname(__DIR__, 2).'/PromptHooks/Provider',
            dirname(__DIR__, 2).'/PromptHooks/Output',
        ];
        foreach ($dirs as $dir) {
