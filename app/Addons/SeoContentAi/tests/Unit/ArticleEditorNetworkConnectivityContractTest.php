 ...
 *
 * Covers required behaviors A–G as static source contracts (remote-first).
 */
final class ArticleEditorNetworkConnectivityContractTest extends TestCase
{
    ...
    {
        $path = dirname(__DIR__, 2).'/resources/views/'.$relative;
    ...
    public function test_network_module_classifies_errors_and_avoids_http_as_offline(): void
    {
        ...
        // HTTP status present ⇒ not network offline.
    ...
    public function test_editor_blocks_autosave_spam_and_updates_status_labels(): void
    {
        ...
        self::assertStringContainsString('Chưa lưu — mất kết nối', $i18n);
        self::assertStringContainsString('Đã kết nối lại — đang lưu thay đổi…', $i18n);
        self::assertStringContainsString('Mất kết nối mạng — các thay đổi hiện chưa được lưu.', $i18n);
        self::assertStringContainsString('Không thể lưu khi đang mất kết nối.', $i18n);
    }
    ...
    public function test_banner_and_sync_disable_wired_in_shell(): void
    {
        ...
        self::assertStringContainsString('Mất kết nối mạng — các thay đổi hiện chưa được lưu.', $edit);
        self::assertStringContainsString('__SEO_EDITOR_NETWORK_STATUS__?.unavailable', $edit);
        self::assertStringContainsString('Không thể lưu khi đang mất kết nối.', $edit);
        ...
        self::assertStringContainsString('Không thể lưu khi đang mất kết nối.', $shell);
    }
