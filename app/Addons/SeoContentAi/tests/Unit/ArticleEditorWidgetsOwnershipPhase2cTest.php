...
use App\Addons\SeoContentAi\Services\ArticleEditor\ArticleEditorFaqSnapshotService;
use App\Addons\SeoContentAi\Services\ArticleFaqGeneratorService;
use PHPUnit\Framework\TestCase;
use ReflectionClass;

/**
 * Phase 2C — FAQ/CTA widget ownership + insertion contracts.
 */
final class ArticleEditorWidgetsOwnershipPhase2cTest extends TestCase
{
    ...
    public function test_frontend_faq_snapshot_and_cta_ownership_wiring(): void
    {
        $faqClient = (string) file_get_contents(
            dirname(__DIR__, 2).'/resources/js/utils/articleEditorFaqSnapshot.js',
        );
        ...
        $faqEditor = (string) file_get_contents(
            dirname(__DIR__, 2).'/resources/js/components/ArticleFaqEditor.jsx',
        );
        ...
        $cta = (string) file_get_contents(
            dirname(__DIR__, 2).'/resources/js/components/CtaContactInsertList.jsx',
        );
    ...
    {
        $path = dirname(__DIR__, 5).'/docs/architecture/ARTICLE_EDITOR_WIDGETS_OWNERSHIP.md';
