...
use App\Addons\SeoContentAi\Enums\Gsc\GscBrandQueryType;
use App\Addons\SeoContentAi\Enums\Gsc\GscContentAction;
use App\Addons\SeoContentAi\Enums\Gsc\GscPageMappingMethod;
...
use App\Addons\SeoContentAi\Services\GscIntelligence\GscBrandQueryClassifier;
use App\Addons\SeoContentAi\Services\GscIntelligence\GscContentActionRecommendationService;
use App\Addons\SeoContentAi\Services\GscIntelligence\GscContentProjectPreviewBuilder;
...
use App\Addons\SeoContentAi\Models\SeoGscDailyMetric;
...
final class GscIntelligenceFoundationTest extends TestCase
{
    ...
    public function test_query_normalization_keeps_vietnamese_diacritics(): void
    {
        ...
        $normalized = $service->normalize('  Dịch Vụ SEO  ');

        self::assertSame('dịch vụ seo', $normalized);
    }
...
2026-07-01,dịch vụ seo,https://example.test/dich-vu-seo,vnm,desktop,,10,100,99,8.5
2026-07-01,seo là gì,https://example.test/seo-la-gi,vnm,mobile,,150,100,1.5,4
CSV;
    ...
    public function test_query_keyword_mapper_blocks_near_duplicate_with_different_intent(): void
    {
        ...
        $candidates = [
            ['site_id' => '1', 'keyword_ref' => 'kw_1', 'normalized' => 'seo là gì'],
        ];

        $mapped = $mapper->map('dịch vụ seo', '1', $candidates);
    ...
    public function test_brand_classifier(): void
    {
        ...
        self::assertSame(GscBrandQueryType::Unknown, $classifier->classify('dịch vụ seo'));
    ...
    {
        $hash = new GscFactHashService;
        $a = $hash->dataHash('prop_1', '2026-07-01', 'dịch vụ seo', 'https://example.test/a', 'vnm', 'desktop', 'none');
        $b = $hash->dataHash('prop_1', '2026-07-01', 'dịch vụ seo', 'https://example.test/a', 'vnm', 'desktop', 'none');
        $c = $hash->dataHash('prop_1', '2026-07-01', 'seo là gì', 'https://example.test/a', 'vnm', 'desktop', 'none');
    ...
    {
        $dir = dirname(__DIR__, 2).'/Services/GscIntelligence/Application/Handlers';
        if (! is_dir($dir)) {
