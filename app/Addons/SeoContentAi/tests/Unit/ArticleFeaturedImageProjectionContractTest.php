...
/**
 * Canonical featured projection for Article List — DB columns, no WP HTTP, no wp_post_images.
 */
final class ArticleFeaturedImageProjectionContractTest extends TestCase
{
    ...
    {
        $path = dirname(__DIR__, 2).'/database/migrations/2026_08_05_110000_add_featured_image_projection_to_articles_table.php';
    ...
    public function test_list_select_includes_projection_not_body(): void
    {
        $resource = (string) file_get_contents(
            dirname(__DIR__, 2).'/Filament/Resources/ArticleResource.php',
        );
    ...
    public function test_list_for_list_reads_attributes_only_no_wp_post_images(): void
    {
        $resolver = (string) file_get_contents(
            dirname(__DIR__, 2).'/Support/ArticleFeaturedImageResolver.php',
        );
        ...
        // Forbid reading content-image gallery meta as featured (call/key only — not comments).
    ...
    public function test_backfill_command_is_idempotent_and_db_only(): void
    {
        $cmd = (string) file_get_contents(
            dirname(__DIR__, 2).'/Console/BackfillArticleFeaturedImageProjectionCommand.php',
        );
    ...
    public function test_unknown_placeholder_distinct_from_absent_in_view(): void
    {
        $view = (string) file_get_contents(
            dirname(__DIR__, 2).'/resources/views/filament/tables/columns/article-list-thumbnail.blade.php',
        );
    ...
    public function test_status_and_source_constants_cover_policy(): void
    {
        $status = (string) file_get_contents(
            dirname(__DIR__, 2).'/Support/ArticleFeaturedImageStatus.php',
        );
        $source = (string) file_get_contents(
            dirname(__DIR__, 2).'/Support/ArticleFeaturedImageSource.php',
        );
