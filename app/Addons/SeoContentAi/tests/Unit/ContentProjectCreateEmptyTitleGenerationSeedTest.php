 ...
 * Asserts VARIABLES / mapped input / compile mirrors — not AI output.
 */
final class ContentProjectCreateEmptyTitleGenerationSeedTest extends TestCase
{
    ...
    {
        $vars = $this->applyOptionalInputs(true, [], 'thời trang bền vững', "  \t  ");
    ...
    {
        $vars = $this->applyOptionalInputs(true, [], 'focus kw', 'Tiêu đề riêng');
    ...
    public function test_rewrite_keeps_article_title_over_keyword_fallback(): void
    {
        $vars = $this->applyOptionalInputs(
            isNewArticle: false,
            existing: [
                'post_title' => 'Xưởng May Balo Xuất Khẩu Uy Tín',
                'title' => 'Xưởng May Balo Xuất Khẩu Uy Tín',
                'focus_keyword' => 'stale',
            ],
            keyword: 'Xưởng May Balo Xuất Khẩu',
            title: '',
        );

        self::assertSame('Xưởng May Balo Xuất Khẩu Uy Tín', $vars['post_title'] ?? null);
        self::assertSame('Xưởng May Balo Xuất Khẩu Uy Tín', $vars['title'] ?? null);
        self::assertSame('Xưởng May Balo Xuất Khẩu', $vars['focus_keyword'] ?? null);
        self::assertSame('Xưởng May Balo Xuất Khẩu', $vars['keyword'] ?? null);
        self::assertSame('Xưởng May Balo Xuất Khẩu Uy Tín', $vars['topic'] ?? null);
    }
    ...
    public function test_outline_map_seeds_post_title_from_keyword_when_empty(): void
    {
        ...
        $mapped = $map->invoke($executor, $fields, [
            'focus_keyword' => 'thời trang bền vững',
        ], []);
        $mapped = $seed->invoke($executor, $mapped, $fields);
        $mapped = $topic->invoke($executor, $mapped, $fields);

        self::assertSame('thời trang bền vững', $mapped['keyword'] ?? null);
        self::assertSame('thời trang bền vững', $mapped['post_title'] ?? null);
        self::assertSame('thời trang bền vững', $mapped['topic'] ?? null);
    }
    ...
    public function test_compile_alias_mirrors_keyword_focus_and_title(): void
    {
        ...
        $out = $method->invoke($executor, [
            'keyword' => 'thời trang bền vững',
            'post_title' => 'thời trang bền vững',
            'topic' => 'thời trang bền vững',
        ]);

        self::assertSame('thời trang bền vững', $out['keyword'] ?? null);
        self::assertSame('thời trang bền vững', $out['focus_keyword'] ?? null);
        self::assertSame('thời trang bền vững', $out['post_title'] ?? null);
        self::assertSame('thời trang bền vững', $out['title'] ?? null);
        self::assertSame('thời trang bền vững', $out['topic'] ?? null);
    }
    ...
    public function test_site_description_alias_mapped_from_short_description(): void
    {
        ...
        $mapped = $map->invoke($executor, $fields, [
            'site_short_description' => 'Shop balo xuất khẩu',
            'keyword' => 'balo',
        ], []);
        $compile = $expand->invoke($executor, $mapped);

        self::assertSame('Shop balo xuất khẩu', $mapped['site_description'] ?? null);
        self::assertSame('Shop balo xuất khẩu', $compile['site_short_description'] ?? null);
        self::assertSame('Shop balo xuất khẩu', $compile['site_description'] ?? null);
    }
