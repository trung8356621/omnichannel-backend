...
use App\Addons\SeoContentAi\Services\Notifications\Publishers\PromptContractNotificationPublisher;
...
final class OperationalNotificationContractTest extends TestCase
{
    ...
    private function readCore(string $relative): string
    {
        ...
        $path = dirname(__DIR__, 5).DIRECTORY_SEPARATOR.str_replace(['/', '\\'], DIRECTORY_SEPARATOR, $relative);
        self::assertFileExists($path);
        $body = file_get_contents($path);
        self::assertIsString($body);
