...
final class ContentProjectApplicationApiFoundationTest extends TestCase
{
    ...
    public function test_command_bus_registrar_covers_core_commands(): void
    {
        $source = (string) file_get_contents(
            (string) (new ReflectionClass(
                \App\Addons\SeoContentAi\Services\ContentProject\Application\ContentProjectCommandBusRegistrar::class
            ))->getFileName()
        );
    ...
    {
        $path = dirname(__DIR__, 2).'/Http/Controllers/Api/V1/ContentProjectApiController.php';
    ...
    {
        $path = dirname(__DIR__, 2).'/Services/ContentProject/Application/Handlers/ProcessScheduledProjectItemPublishHandler.php';
        $source = (string) file_get_contents($path);
