...
use App\Addons\SeoContentAi\Services\ArticleEditor\Document\InlineMarkBoundaryWhitespace;
...
use App\Addons\SeoContentAi\Services\ContentProject\Publishing\PublishingQueueRemoteReconcileService;
...
final class WordpressPublishWhitespaceAndCompletionContractTest extends TestCase
{
    public function test_inline_spaces_preserved_for_strong_em_anchor(): void
    {
        ...
        $cases = [
            'phải <strong>thông minh</strong> trong cách lựa chọn',
            'lý do <strong>phong cách Athleisure</strong> ra đời',
            'nơi <em>sự thoải mái</em> kết hợp cùng vẻ ngoài',
            'xem <a href="/x">bài liên quan</a> tại đây',
        ];
    ...
    public function test_repair_unglues_word_mark_boundaries_without_punct_space(): void
    {
        $repair = new InlineMarkBoundaryWhitespace;
        self::assertSame(
            'phải <strong>thông minh</strong> trong',
            $repair->repair('phải<strong>thông minh</strong>trong'),
        );
        self::assertSame(
            '<strong>Từ khóa</strong>, ví dụ',
            $repair->repair('<strong>Từ khóa</strong>, ví dụ'),
        );
    }
    ...
    public function test_dispatch_success_outcome_is_not_published_constant(): void
    {
        ...
        $runner = (string) file_get_contents(
            dirname(__DIR__, 2).'/Services/ContentProject/ContentProjectPublishingQueueRunner.php',
        );
    ...
    public function test_reconcile_service_classifies_explicitly(): void
    {
        ...
        $cmd = (string) file_get_contents(
            dirname(__DIR__, 2).'/Console/ReconcilePublishingQueueTasksCommand.php',
        );
    ...
    {
        $repair = new InlineMarkBoundaryWhitespace;
        $glued = '<p>phải<strong>thông minh</strong>trong cách lựa chọn lý do<strong>phong cách Athleisure</strong>ra đời</p>';
        $fixed = $repair->repair($glued);
        self::assertStringContainsString('phải <strong>thông minh</strong> trong', $fixed);
        self::assertStringContainsString('lý do <strong>phong cách Athleisure</strong> ra đời', $fixed);
        self::assertStringNotContainsString('phải<strong>', $fixed);
    ...
    {
        $src = (string) file_get_contents(
            dirname(__DIR__, 2).'/Services/ArticleEditorHtmlSanitizeService.php',
        );
    ...
    {
        $cmd = (string) file_get_contents(
            dirname(__DIR__, 2).'/Console/PublishScheduledArticlesCommand.php',
        );
