...
use App\Addons\SeoContentAi\Support\ArticleReviewCutoverRules;
...
/**
 * Batch C hard cutover — review_status sole SoT, is_reviewed removed.
 */
final class ContentProjectApprovalSotTest extends TestCase
{
    ...
    public function test_project_approve_handler_is_all_or_nothing(): void
    {
        ...
        // Import + emit — assert single dispatch after commit only.
    ...
    {
        $path = dirname(__DIR__, 2).'/database/migrations/2026_07_31_120000_cutover_drop_articles_is_reviewed.php';
    ...
    public function test_publish_eligibility_uses_lifecycle_approved_from_review_status(): void
    {
        ...
        $queue = $this->source(
            \App\Addons\SeoContentAi\Services\ContentProject\ContentProjectPublishingQueueService::class,
        );
        // Batch D+: schedule eligibility = ActionGuard (resolver → review_status), not raw resolvePhase.
