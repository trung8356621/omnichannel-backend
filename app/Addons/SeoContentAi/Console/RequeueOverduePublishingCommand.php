...
final class RequeueOverduePublishingCommand extends Command
{
    ...
    public function handle(
        ...
        \App\Addons\SeoContentAi\Services\ContentProject\Publishing\PublishingStuckRecoveryService $stuckRecovery,
    ): int {
        ...
        if ($forceSelected !== '') {
            ...
            foreach ($byProject as $pid => $tasks) {
                $project = \App\Addons\SeoContentAi\Models\SeoProject::query()->find((int) $pid);
                if (! $project) {
                    continue;
                }
