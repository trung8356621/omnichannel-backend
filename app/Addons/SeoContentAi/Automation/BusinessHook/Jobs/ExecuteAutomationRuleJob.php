...
namespace App\Addons\SeoContentAi\Automation\BusinessHook\Jobs;
...
final class ExecuteAutomationRuleJob implements ShouldQueue
{
    ...
    {
        try {
            $execution = \App\Addons\SeoContentAi\Automation\BusinessHook\Models\AutomationExecution::query()
                ->with('rule')
                ->find($this->automationExecutionId);

            if ($execution?->rule?->isGraphMode()) {
                app(\App\Addons\SeoContentAi\Automation\BusinessHook\Services\AutomationGraphExecutionService::class)
