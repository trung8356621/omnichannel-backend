@php
    $canRestoreFromWordPress = ! \App\Addons\SeoContentAi\Support\SeoAccessControl::isContentManager()
    ...
    $dangerActionLabels = [
        ...
        'delete_heading' => __('filament-actions::delete.single.modal.heading'),
        'delete_description' => __('filament-actions::delete.single.modal.description'),
        'delete_submit' => __('filament-actions::delete.single.modal.actions.delete.label'),
        'cancel' => __('filament-actions::modal.actions.cancel.label'),
    ];
@endphp
        ...
        window.seoArticleRestoreAction = function seoArticleRestoreAction(config) {
            ...
            return {
                ...
                async confirmRestore() {
                    ...
                    try {
                        const ok = await wire.call('restoreArticleFromWordPress');
                        if (!ok) {
...
@if ($canRestoreFromWordPress)
    ...
    >
        <button
            ...
            class="fi-btn relative grid-flow-col items-center justify-center font-semibold outline-none transition duration-75 focus-visible:ring-2 fi-btn-size-md fi-color-info gap-1.5 px-3 py-2 text-sm inline-grid shadow-sm bg-custom-50 text-custom-600 hover:bg-custom-100 dark:bg-custom-400/10 dark:text-custom-400 dark:hover:bg-custom-400/20 ring-1 ring-custom-600/10 dark:ring-custom-400/30"
            x-bind:disabled="restoreBusy"
            x-on:click="openRestoreModal()"
            x-bind:title="labels.restore"
            x-bind:aria-label="labels.restore"
        >
            <svg class="fi-btn-icon h-5 w-5" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" aria-hidden="true">
            ...
            <span class="fi-btn-label" x-text="labels.restore"></span>
        </button>
...
>
    <button
        ...
        class="fi-icon-btn relative flex items-center justify-center rounded-lg outline-none transition duration-75 focus-visible:ring-2 fi-color-danger fi-size-md h-9 w-9 text-danger-600 hover:bg-danger-50 dark:text-danger-400 dark:hover:bg-danger-400/10"
