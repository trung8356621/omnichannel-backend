...
final class ArticleFeaturedImageResolver
{
    ...
    public function forList(SeoArticle $article): array
    {
        ...
        $status = trim((string) ($attrs['featured_image_status'] ?? ''));
        if ($status === '' || ! in_array($status, ArticleFeaturedImageStatus::all(), true)) {
            $status = ArticleFeaturedImageStatus::UNKNOWN;
        }

        $url = trim((string) ($attrs['featured_thumb_url'] ?? ''));
        $mediaId = isset($attrs['featured_media_id']) && $attrs['featured_media_id'] !== null
            ? (int) $attrs['featured_media_id']
            : null;
        $source = trim((string) ($attrs['featured_image_source'] ?? ''));
