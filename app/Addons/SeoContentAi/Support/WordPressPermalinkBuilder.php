...
final class WordPressPermalinkBuilder
{
    ...
    {
        $article->loadMissing('articleMetas');
        $fromMeta = trim((string) ($article->articleMetas->firstWhere('meta_key', 'wp_post_type')?->meta_value ?? ''));

        return $fromMeta !== '' ? $fromMeta : strtolower(trim((string) ($article->type ?? '')));
    }
