...
use App\Addons\SeoContentAi\Models\SeoProjectTaskEvent;
    ...
    private function repairArchiveMirrors(bool $apply, ?int $projectId, array &$stats): void
    {
        ...
        foreach ($archived->cursor() as $task) {
            ...
            if ($mirror instanceof SeoContentArchiveItem) {
                ...
                SeoArticle::query()->whereKey($articleId)->whereNull('content_archived_at')->update([
                    'content_archived_at' => $task->archived_at ?? now(),
        ...
        // Active task + mirror: remove mirror / clear flags
        ...
        foreach ($mirrors->cursor() as $mirror) {
            ...
            $mirror->delete();
            if ($articleId > 0) {
                $stillArchived = SeoProjectTask::query()
                    ->archived()
