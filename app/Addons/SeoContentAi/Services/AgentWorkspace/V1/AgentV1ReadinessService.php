...
namespace App\Addons\SeoContentAi\Services\AgentWorkspace\V1;
...
use App\Addons\SeoContentAi\Services\AgentWorkspace\Packs\AgentPackCache;
use App\Addons\SeoContentAi\Services\AgentWorkspace\Packs\AgentPackRegistry;
    ...
    private function checkP0Capabilities(): array
    {
        ...
        foreach (array_unique($required) as $cap) {
            ...
            if ($this->capabilities->get($cap) === null
                && ! in_array($cap, \App\Addons\SeoContentAi\Services\ContentProject\Agent\ContentProjectAgentGateway::READ_CAPABILITIES, true)) {
                $missing[] = $cap;
            }
        }
