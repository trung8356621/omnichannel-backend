...
final class ArticleCompletedArchiveQueryService
{
    ...
    {
        if ($article->content_archived_at instanceof Carbon) {
            return $article->content_archived_at;
        }
    ...
    public function resolveCompletedByLabel(SeoArticle $article): string
    {
        ...
        $userId = (int) ($article->content_archived_by ?? 0);
        if ($userId <= 0) {
            return '—';
        }
    ...
    public function monthOptionsForSites(array $siteIds): array
    {
        ...
        $months = $this->queryForSites($siteIds)
            ->whereNotNull('content_archived_at')
            ->selectRaw("DATE_FORMAT(content_archived_at, '%Y-%m') as month_key")
            ->distinct()
    ...
    public function buildGroupedDashboard(array|int $siteIds): array
    {
        ...
        $articles = $this->queryForSites($normalizedSiteIds)
            ...
            ->orderByDesc('content_archived_at')
            ->orderByDesc('id')
            ->get();
