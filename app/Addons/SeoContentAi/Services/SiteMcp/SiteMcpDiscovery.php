...
namespace App\Addons\SeoContentAi\Services\SiteMcp;
...
use App\Addons\SeoContentAi\Services\SiteSync\Support\SiteSyncInfrastructure;
use App\Models\Site;
use Throwable;
    ...
    private function discoverCatalog(Site $site): array
    {
        ...
        try {
            $articles = SeoArticle::query()
                ...
                ->orderBy('id')
                ->limit(2000)
                ->get(['id', 'title', 'slug', 'type', 'site_id', 'wp_post_id']);
        } catch (Throwable) {
            return $empty;
        }
