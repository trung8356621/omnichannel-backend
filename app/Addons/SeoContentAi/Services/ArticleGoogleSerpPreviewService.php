...
final class ArticleGoogleSerpPreviewService
{
    ...
    public function buildForArticle(
        ...
        $rawSchema = $this->resolveStoredSchemaJson($article);
        $parsed = $this->schemaParser->parse(
            $rawSchema !== '' ? $rawSchema : $this->syntheticSchemaJson($article, $seoTitle, $seoDescription, $permalink),
        );
    ...
    private function resolveStoredSchemaJson(SeoArticle $article): string
    {
        foreach ([
            'rank_math_schema_json',
            'rankmath_schema_json',
            'json_ld',
            'schema_json',
        ] as $key) {
