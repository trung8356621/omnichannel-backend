...
namespace App\Addons\SeoContentAi\Services\SiteSync\Comparison;
...
use App\Addons\SeoContentAi\Models\SiteSync\SeoSiteSyncComparisonDiff;
use App\Addons\SeoContentAi\Models\SiteSync\SeoSiteSyncComparisonRun;
...
final class SiteSyncComparisonService
{
    ...
    private function compareArticles(Site $site, int $limit): array
    {
        ...
        $articles = SeoArticle::query()
            ...
            ->orderBy('id')
            ->limit($limit)
            ->get(['id', 'wp_post_id', 'title', 'status', 'slug']);

        foreach ($articles as $article) {
            if ((int) $article->wp_post_id <= 0) {
