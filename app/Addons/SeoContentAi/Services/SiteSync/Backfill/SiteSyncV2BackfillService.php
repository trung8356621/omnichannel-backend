...
namespace App\Addons\SeoContentAi\Services\SiteSync\Backfill;
...
use App\Addons\SeoContentAi\Services\SiteSync\Reconciliation\KeywordNormalizationService;
use App\Addons\SeoContentAi\Services\SiteSync\Reconciliation\SiteLinkCatalogReconciler;
...
final class SiteSyncV2BackfillService
{
    ...
    private function backfillLinks(Site $site, bool $dryRun): array
    {
        ...
        foreach ($rows as $row) {
            ...
            $exists = \App\Addons\SeoContentAi\Models\SiteSync\SeoSiteManualLink::query()
        ...
        foreach ($articles as $article) {
            ...
            if (! $dryRun) {
                SeoArticleScoreSource::query()->create([
                    ...
                    'wordpress_id' => $article->wp_post_id,
                    'source' => SiteSyncSchema::SOURCE_LEGACY_UNKNOWN,
                    'score' => (int) $article->seo_score,
