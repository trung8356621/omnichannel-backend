...
use App\Addons\SeoContentAi\Enums\KeywordMetricStatus;
use App\Addons\SeoContentAi\Filament\Resources\AiConnectionResource;
use App\Addons\SeoContentAi\Models\KeywordGroupMetricSnapshot;
...
final class SeoPerformanceDashboardService
{
    ...
    public function isKeywordMetricsSource(string $source): bool
    {
        ...
        return $provider === \App\Addons\SeoContentAi\Support\ApiConnectionProviders::KEYWORDS_EVERYWHERE;
    }
    ...
    {
        $provider = $this->providerRegistry->resolveProviderFromSource($source);
        if ($provider !== \App\Addons\SeoContentAi\Support\ApiConnectionProviders::KEYWORDS_EVERYWHERE) {
        ...
        foreach ($definition->dashboardSections as $section) {
            if ($section !== PerformanceHubSectionKey::IntegrationState) {
                $hasImplementedMetrics = $definition->isCapabilityImplemented(\App\Addons\SeoContentAi\Enums\SeoProviderCapabilityKey::SearchVolume);
                break;
            }
        }
