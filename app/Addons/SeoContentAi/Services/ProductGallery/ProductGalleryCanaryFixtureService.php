...
final class ProductGalleryCanaryFixtureService
{
    ...
    public static function inputRequirements(): array
    {
        return [
            'required' => [
                'post_type' => 'Must be product (article.type + wp_post_type).',
    ...
    public function create(
        ...
        return DB::connection('omi_seo_ai')->transaction(function () use (
            ...
            try {
                $projectUrl = \App\Addons\SeoContentAi\Filament\Resources\SeoProjectResource::getUrl('edit', ['record' => $project->id]);
            } catch (\Throwable) {
                $projectUrl = '/seo/seo-projects/'.$project->id.'/edit';
            }
            try {
                $canaryPageUrl = \App\Addons\SeoContentAi\Filament\Pages\ProductGalleryCanaryPage::getUrl([
