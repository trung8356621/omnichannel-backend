...
final class ArticleWpSyncLeaseService
{
    ...
    public function resetArticle(SeoArticle $article, string $reason = 'Cancelled manually'): bool
    {
        ...
        DB::connection('omi_seo_ai')->transaction(function () use ($article, $articleId, $reason, &$changed): void {
            ...
            $updates = [];
            if (
                Schema::connection('omi_seo_ai')->hasColumn('articles', 'wp_sync_status')
                && (string) ($fresh->wp_sync_status ?? '') !== self::ARTICLE_IDLE
            ) {
                $updates['wp_sync_status'] = self::ARTICLE_IDLE;
            }
            if (
                Schema::connection('omi_seo_ai')->hasColumn('articles', 'wp_sync_job_id')
                && $fresh->wp_sync_job_id !== null
            ) {
    ...
    private function writeOrphanMetaTerminal(
        ...
        $updates = [];
        if (Schema::connection('omi_seo_ai')->hasColumn('articles', 'wp_sync_status')) {
            $updates['wp_sync_status'] = self::ARTICLE_IDLE;
        }
        if (Schema::connection('omi_seo_ai')->hasColumn('articles', 'wp_sync_job_id')) {
            $updates['wp_sync_job_id'] = null;
        }
        if ($updates !== []) {
    ...
    private function bindArticle(SeoArticle $article, SeoArticleWpSyncJob $job, WpSyncJobStatus $status): void
    {
        $updates = [];
        if (Schema::connection('omi_seo_ai')->hasColumn('articles', 'wp_sync_status')) {
            $updates['wp_sync_status'] = $status->value;
        }
        if (Schema::connection('omi_seo_ai')->hasColumn('articles', 'wp_sync_job_id')) {
            $updates['wp_sync_job_id'] = (int) $job->id;
        }
        if ($updates !== []) {
            $article->forceFill($updates)->save();
        }
