...
final class ProductGalleryPromptHookBindingTest extends TestCase
{
    ...
    {
        $dir = dirname(__DIR__, 4).'/addons/ai-prompt'.'/resources/prompt-hooks/v01';
        foreach ([
    ...
    public function test_installer_does_not_overwrite_existing_binding(): void
    {
        $source = (string) file_get_contents(
            dirname(__DIR__, 4).'/addons/ai-prompt'.'/Services/PromptOwnership/DefaultProductGalleryPromptsInstaller.php',
        );
    ...
    {
        $path = dirname(__DIR__, 4).'/addons/ai-prompt'.'/resources/prompt-hooks/v01/product.gallery.plan@0.1.0.json';
        $json = json_decode((string) file_get_contents($path), true);
    ...
    public function test_parent_and_child_hooks_declare_identity_fields(): void
    {
        foreach (['parent', 'child'] as $which) {
            ...
            $file = $which === 'parent'
                ...
                : 'product.gallery.child.generate@0.1.0.json';
            $json = json_decode(
                (string) file_get_contents(dirname(__DIR__, 4).'/addons/ai-prompt'.'/resources/prompt-hooks/v01/'.$file),
                true,
            );
