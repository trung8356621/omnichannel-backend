...
Single execution path: Filament / Publishing Queue / Scheduler / REST / Agent → `ContentProjectCommandBus`.
...
| Public refs | `ContentProjectPublicRef` — API uses **strict** opaque refs |
| Publisher | `ContentPublisher` → `WordPressContentPublisher` |
...
HTTP: 200/201 success · 202 processing · 409 lock/lifecycle/confirmation · 422 validation · 403 forbidden · 404 not found · 429 quota · 503 wordpress unavailable.
