...
**Not registered:** live Google Search Analytics HTTP/SDK provider (legacy path remains `GoogleSearchConsoleSyncService` → SiteMeta snapshot).
...
Handlers under `Application/Handlers/` must not import `Google\Client` or `Google_Service` — provider boundary only.
