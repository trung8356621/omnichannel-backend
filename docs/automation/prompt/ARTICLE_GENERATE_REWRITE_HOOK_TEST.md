...
1. Prompt create/edit → dropdown shows:
   - **Viết bài viết (Thử nghiệm)**
   - **Viết lại bài viết (Thử nghiệm)**
2. Select Generate → save `hook_key=article.content.generate`, `hook_version=0.1.0`.
3. Markdown editor stays editable; note: Hook manages contract/runtime; Prompt content still sent to AI.
4. Select Rewrite → save exact key/version.
5. Clear Hook → null binding → legacy PromptRunner.
...
| outline + vocabulary as `{{input}}` | Combined planning → full article |
