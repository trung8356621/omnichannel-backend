# Prompt Workflow Inventory — SeoContentAi

**Phase:** 5A  
**Updated:** 2026-07-18  
**Scope:** Audit only — không rewrite engine, không đổi production mode.

## Classification legend

| Class | Meaning |
|---|---|
| KEEP_AS_IS | Ổn / orchestration cần giữ |
| NORMALIZE_NOW | Contract rõ; fixture normalize trong 5A |
| MIGRATE_LATER | Cần Hook/Workflow engine sau |
| DEPRECATED | Legacy alias / sắp bỏ |
| BLOCKED | WP / incomplete / Filament-bound |
| EXPERIMENTAL | Hook thử nghiệm — chưa khóa contract engine |
...
| `GlobalAiChatService` | (chat) | Hardcoded system prompt — ngoài PromptRunner |
| `AiKeywordDiscoveryService` | (discovery) | Hardcoded JSON system — ngoài PromptRunner |
...
| `post_review_task_id` | SeoTask workflow | BLOCKED | `post_comment_review` → WP path |
...
Tone/length: `SeoPromptSettingsService` (`seo_prompt_settings`) — KEEP_AS_IS (context inject).
...
|---|---|---|
| `article.title_suggestion` | EXPERIMENTAL | Runtime Active UI; contract engine chưa khóa |
| `article.meta_description_suggestion` | EXPERIMENTAL | Runtime Active UI; contract engine chưa khóa |
...
| `AiKeywordDiscoveryService` system_instruction | NORMALIZE_NOW → fixture `keyword.discovery.structured` |
...
DB `prompts` / `seo_tasks` rows: dynamic (tenant data) — không đếm cứng; inventory theo **binding + caller family**.
...
Article Editor save · WP sync · delete legacy prompts · full Workflow Builder · production AI calls.
