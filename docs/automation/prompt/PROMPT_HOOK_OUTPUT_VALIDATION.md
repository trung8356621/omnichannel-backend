...
Pipeline: raw → normalize steps → parse → schema validate → DTO.
...
Parser: `MarkdownSectionsOutputParser` — definition-driven (reads `output_schema.sections`). Escapes markers before regex. No hardcoded hook-key branches.
...
Typed failures: `MissingRequiredSection`, `DuplicateOutputSection`, `MismatchedSectionMarker`, `UnknownSectionMarker`, `TextOutsideDeclaredSections`, `InvalidSectionOutput` — messages include hook key/version, section key, expected markers, `correlation_id`. Do not show full AI response / secrets in production UI.
...
`article.content.generate` and `article.content.rewrite` use `type: markdown` with `minimum_length`, optional `reject_provider_preamble`, and `reject_task_markers=false` for generate. Normalize: `trim`, `strip_markdown_fence`. No Markdown→HTML in Hook Runtime.

Retry: Hook runtime classifies validation failure only. Retry ownership remains PromptRunner / AiModelRouter — no second retry loop in the parser.
