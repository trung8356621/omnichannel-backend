...
| Class | Role |
|---|---|
| `Canonical\PromptHookDefinition` (+ Key/Version/Status/…) | Immutable definition |
| `Runtime\PromptHookDefinitionLoader` | Spec v0.1 + Phase 1 dual-read |
| `Runtime\PromptHookRuntimeRegistry` | `get(key, version)` — no latest for experimental |
...
Workflow mapping (existing nodeOutputs): section ports → Task 1/2; `total` → `out_main` / Total (AI). No new workflow engine.

## legacy_prompt_content (article generate/rewrite)

`article.content.generate@0.1.0` and `article.content.rewrite@0.1.0` use Prompt DB markdown as template; Hook JSON owns I/O contract only. Output type `markdown` → workflow `out_main` / Total (AI). Domain persist stays outside Hook Runtime.

## Config

`config/seo-content-ai.php` → `prompt_hooks.*`  
