...
Analyzer: `SerpContentGapAnalyzer` — version `CONTENT_GAP_ANALYZER_VERSION = 1.0.0`.
...
| `missing_heading` | H2 section on ≥2 competitors, missing on own |

## Rules

- Requires `min_frequency` (default 0.3) across competitor set
- **Single heading on one competitor does not create strong gap** — section gaps need ≥2 competitor hits + frequency threshold
...
`SerpEvidenceContentProjectPreviewAdapter` merges `serp_evidence` into preview items — no `gallery_description`.
