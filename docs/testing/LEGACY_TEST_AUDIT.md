...
**Không** chạy `php artisan test:ci`. Dùng `composer test:ci` hoặc `php scripts/...`.

Bật `ext-fileinfo` trên PHP 8.3 CLI trước `composer install`. Giữ `composer.lock`.
...
|--------|------:|---------|--------|
| ENV_MYSQL | 25 | `SQLSTATE[HY000] [2002]` MySQL refused | FIX_INFRASTRUCTURE — TestCase sqlite remap (default); server set `SEO_TEST_USE_MYSQL=true` when DB up |
| CONFIG | 8 | `Target class [config] does not exist` | FIX_INFRASTRUCTURE — switch to `Tests\TestCase` |
| FINAL (heuristic) | ~6–20 | Mockery/PHPUnit mock on `final` class **or** source assert containing word `final` | UPDATE_STALE / don't mock final |
...
| Change | Why |
|--------|-----|
| `tests/TestCase.php` — remap `core_connection` + `omi_seo_ai` to default sqlite unless `SEO_TEST_USE_MYSQL=true` | Stop mass ENV_MYSQL cascade without live MySQL |
...
| 6 CONFIG tests → `Tests\TestCase` | ArticleMetaMap, PostTypeResolver, WorkflowParser, LinkSuggestion*, ProductReviewAutomationPublish |
...
## File inventory (failing Seo Unit — action map)
...
### ENV_MYSQL → FIX_INFRASTRUCTURE (keep tests; rely on TestCase / server MySQL)
...
### CONFIG → FIX_INFRASTRUCTURE (switched to Tests\TestCase)

| File | Status | Root cause | Action | Evidence |
|------|--------|------------|--------|----------|
| ArticleMetaMapTest.php | FIX_INFRASTRUCTURE | PHPUnit TC + Eloquent/config | Switched → Tests\TestCase; **pass** spot-check | `Support/ArticleMetaMap.php` |
...
### FINAL mock → UPDATE_STALE (do not delete; stop mocking finals)
...
### SKIP files (existing skips — do not add more)
...
Do **not** mass-delete on “failing” alone.
...
| Failures | 43 | 44 |
| ENV_MYSQL files | 25 | 20 → **0 connection-refused** after `refreshApplication()` remap (spot: KeywordPersistence now `no such table` = sqlite without SEO migrations) |
| CONFIG files | 8 | 1 |

**Infra lesson:** `afterApplicationCreated` chạy **sau** `setUpTraits` → quá muộn cho `DatabaseTransactions`. Remap trong `refreshApplication()`.

Local without MySQL: connection OK (sqlite) nhưng thiếu schema SEO → wave 2 migrate `app/Addons/SeoContentAi/database/migrations` lên `omi_seo_ai` sqlite **hoặc** server dùng `SEO_TEST_USE_MYSQL=true`.
...
| PRODUCTION_BUG kept | ≥1 (`PromptExecutionOrchestrationTest`) |
...
5. Revisit SKIP smoke tests → real isolation.
