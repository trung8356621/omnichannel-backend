# Agent Workspace Phase 6 — Observability, Evaluation & Governance

## 1. Inspect findings

- Logging: `RuntimeLogger` (web_app) — reuse for security fallback.
- Metrics pattern: `ContentProjectOpsMetrics` fail-open upsert — mirrored for Agent aggregates.
...
- Retention: Automation cleanup command pattern → `agent:observability:prune`.
- No duplicate global APM — Agent-scoped side-channel only.

## 2. Architecture

```
Decorators (Planning/Execution/Knowledge/Automation)
  → AgentTraceService + AgentMetricRecorder + PolicyDetector
  → EventBus (allowlisted) + RuntimeLogger fallback (security)

Offline: agent:evaluate → AgentEvaluationRunner (NO Phase 2 / CommandBus)
...
## 22. Phase 7 candidates (IMPLEMENTED in Phase 7 — see PHASE_7 handoff)
